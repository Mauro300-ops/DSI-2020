    ...
    class GestionCursos
    {
        ...
        class Sujeto
        {
            ...
            static public void RegistroSujeto()
            {
                ...
                var sujetoJson = JsonConvert.SerializeObject(Sujetos, Formatting.Indented);
                System.IO.File.WriteAllText("Sujeto.Json", sujetoJson);
            }
            ...
            static public void MostrarSujetos()
            {
                ...
                if (System.IO.File.Exists("Sujeto.json"))
                {
                    string ArchivoSujeto = System.IO.File.ReadAllText("Sujeto.json");
                    List<Sujeto> sujetoJson = JsonConvert.DeserializeObject<List<Sujeto>>(ArchivoSujeto);

                    int pos = 1;

                    foreach (var sujetos in sujetoJson)
                    {
        ...
        class Inscripcion
        {
            ...
            static public void MostrarInscripciones()
            {
                ...
                {
                    string ArchivoInscripcion = System.IO.File.ReadAllText("inscripcion.json");
                    List<Inscripcion> incripcionJson = JsonConvert.DeserializeObject<List<Inscripcion>>(ArchivoInscripcion);

                    int pos = 1;
                    foreach (var inscripciones in incripcionJson)
                    {
